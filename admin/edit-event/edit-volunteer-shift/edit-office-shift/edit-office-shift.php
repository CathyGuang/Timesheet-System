...
<body>
  ...
  <?php
    ...
    if ($_POST['DELETE']) { //DELETE SHIFT IF DELETE IS REQUESTED
      $query = "DELETE FROM office_shifts WHERE office_shift_type = '{$_POST['old-shift-type']}' AND leader = (SELECT id FROM workers WHERE name LIKE '{$_POST['old-leader']}' AND archived IS NULL) AND archived IS NULL;";
    ...
    if ($_POST['archive']) { //ARCHIVE SHIFT IF REQUESTED
      $query = "UPDATE office_shifts SET archived = 'TRUE' WHERE office_shift_type = '{$_POST['old-shift-type']}' AND leader = (SELECT id FROM workers WHERE name LIKE '{$_POST['old-leader']}' AND archived IS NULL);";
      $result = pg_query($db_connection, $query);
      if ($result) {
        echo "<h3 class='main-content-header'>Success</h3";
      } else {
    ...
    $getShiftIDsQuery = "SELECT DISTINCT office_shifts.id FROM office_shifts, workers WHERE office_shift_type = '{$_POST['old-shift-type']}' AND leader = (SELECT id FROM workers WHERE name LIKE '{$_POST['old-leader']}' AND archived IS NULL) AND archived IS NULL;";
