...
<body>
  ...
  <?php
    $staffName = pg_escape_string(trim($_POST['volunteer']));
    if (empty($staffName)){
  ...
  <form method="post" action="volunteer-csv-download.php">
    <input type="hidden" name="hour_data" value= "<?php echo htmlentities(serialize($hourData)); ?>">
    <button class="blue-button" type="submit">Export Volunteer Hours Data</button>
  </form>
