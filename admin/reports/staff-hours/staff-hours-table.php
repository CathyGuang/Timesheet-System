...
<?php
  ...
  print_r($metadata);
  //Get table data
  $test = pg_field_is_null(pg_query($db_connection, "SELECT id, staff, hours, work_type, date_of_hours, notes FROM staff_hours WHERE work_type = '{$workType}' AND '{$startDate}' <= date_of_hours <= '{$endDate};"));
  echo $test;
