...
<body>
  ...
  <?php
    if ($_POST['selected-shift']) {
      ...
      $startDate = pg_fetch_array(pg_query($db_connection, "SELECT MIN (date_of_shift) AS start_date FROM horse_care_shifts WHERE shift_code = '{$shiftCode}' AND (archived IS NULL OR archived = '');"), 0, 1)['start_date'];
      $endDate = pg_fetch_array(pg_query($db_connection, "SELECT MAX (date_of_shift) AS end_date FROM horse_care_shifts WHERE shift_code = '{$shiftCode}' AND (archived IS NULL OR archived = '');"), 0, 1)['end_date'];
      ...
      <form autocomplete="off" action="edit-horse-care-shift.php" method="post" class="standard-form standard-form">
...
EOT;
          $query = "SELECT DISTINCT care_type, name, shift_code FROM horse_care_shifts, workers WHERE workers.id = horse_care_shifts.leader AND (horse_care_shifts.archived IS NULL OR horse_care_shifts.archived = '');";
          $result = pg_query($db_connection, $query);
